...
const generateSummary = (data: InfoData, { applicationGenerated }: { applicationGenerated: boolean }) => `
| Data | Value |
    ...
    return this.asDefaultTaskGroup({
      async generateSample() {
        ...
        this.destinationRoot(
          this.destinationPath(this.projectFolder ?? join(this._globalConfig.get('samplesFolder'), `issues/${this.issue}`)),
        );
  ...
  get [BaseGenerator.END]() {
    return this.asEndTaskGroup({
      async generateSample() {
        ...
        const { workspacesFolders, jdlEntitiesDefinitions, yoRcContent, jdlDefinitions, jdlApplications = 0, files } = this.data;
        try {
          ...
          setGithubTaskOutput(SUMMARY_OUTPUT, generateSummary(this.data, { applicationGenerated: false }));
          throw error;
        }

        setGithubTaskOutput(RESULT_OUTPUT, SUCCESS);
        setGithubTaskOutput(SUMMARY_OUTPUT, generateSummary(this.data, { applicationGenerated: true }));
